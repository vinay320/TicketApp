...
let rbtn = document.querySelector(".remove-btn");
let mainCont = document.querySelector(".main-cont");
let modalCont = document.querySelector(".modal-cont");
let textareaCont = document.querySelector(".textarea-cont");
let allpc = document.querySelectorAll(".priority-color");
let tbc = document.querySelectorAll(".color");

let colors = ["lightpink", "lightblue", "lightgreen", "black"];
let mpc = colors[colors.length - 1];
...
for(let i = 0 ;  i < tbc.length ; i++){
    tbc[i].addEventListener("click" , (e) =>{
        let ctbc = tbc[i].classList[0];

        let filteredTickets = ticketArr.filter((ticketObj , idx) =>{
            return ctbc === ticketObj.ticketColor;
        })
    ...
    tbc[i].addEventListener("dblclick" , (e) =>{
        let allTicketsCont = document.querySelectorAll(".ticket-cont");
        for(let i=0 ; i < allTicketsCont.length ; i++){
            allTicketsCont[i].remove();
        }
...
allpc.forEach((ColorElem, idx) => {
    ColorElem.addEventListener("click", (e) => {
        allpc.forEach((priorityColorElem, idx) => {
            priorityColorElem.classList.remove("border");
        })
        ColorElem.classList.add("border");

        mpc = ColorElem.classList[0];
    })
})
...
modalCont.addEventListener("keydown", (e) => {
    let key = e.key;
    if (key === "Shift") {
        createTicket(mpc, textareaCont.value);
        addFlag = false;
        setModalToDefault();
    }
})
...
function setModalToDefault(){
    ...
    allpc[allpc.length - 1].classList.add("border");
